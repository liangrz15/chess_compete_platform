...
int GomokuLogic::isVaild(int x, int y, int color)
{
    if(x >= 0 && x < 9 && y >= 0 && y < 9)
    {
...
Player_Color GomokuLogic::win_tie()
{
...
void GomokuLogic::play_chess(int x, int y, Player_Color color)
{
...
void GomokuLogic::save(QString name)
{
    int i;
    QFile file(name);
...
Player_Color GomokuLogic::load(QString name)
{
    int i;
...
bool GomokuLogic::withdraw(Player_Color color)
{
    if(color == Black)
    {
        if(black_num == 0)
...
void GomokuLogic::getBoard(Board* ui_board)
{
    int i,j;
    Player_Color temp;
    for(i = 0;i < 9;i++)
